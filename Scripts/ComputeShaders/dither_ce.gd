...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	push_constants.append(inv_proj_mat[2].w)              # 2
	push_constants.append(inv_proj_mat[3].w)              # 3
	
	# ------------- Pushing Export Variables
	push_constants.append(chromab_polarity)               # 4
	push_constants.append(noise_frequency)                # 5
	push_constants.append(noise_tiling)                   # 6
	push_constants.append(noise_intensity)                # 7
	
	push_constants.append(vignette_intensity)             # 8
	push_constants.append(float(psx_toggle))              # 9
	push_constants.append(dither_scale)                   # 10
	push_constants.append(float(gradient_flip))           # 11
	push_constants.append(gradient_intensity)             # 12
	push_constants.append(time_seconds)                   # 13
	...
	for view in scene_buffers.get_view_count():
		...
		var vignette_uniform_set : RID = UniformSetCacheRD.get_cache(shader, 2, [vignette_uniform])
		...
		var g_uniform_set: RID = rd.uniform_set_create([g_sampler_uniform], shader, 3)
		...
		var d_uniform_set: RID = rd.uniform_set_create([d_sampler_uniform], shader, 4)
		...
		var s_uniform_set : RID = UniformSetCacheRD.get_cache(shader, 5, [s_uniform])
		...
		rd.compute_list_bind_uniform_set(compute_list, vignette_uniform_set, 2)
		rd.compute_list_bind_uniform_set(compute_list, g_uniform_set, 3)
		rd.compute_list_bind_uniform_set(compute_list, d_uniform_set, 4)
		rd.compute_list_bind_uniform_set(compute_list, s_uniform_set, 5)
